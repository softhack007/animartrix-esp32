...
#include <algorithm> // get min/max template
...
static inline float colorburn(float &a, float&b) {  
  ...
  return (1.0f-((1.0f-a/255.f) / (b/255.f)))*255.f;
}
...
static inline float screen(float &a, float&b) {
  ...
  return (1.0f - (1.0f - a/255.f) * (1.0f - b/255.f))*255.f;
}
...
void Chasing_Spirals() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      pixel.blue  = show3 * radial_filter / 4.0f;
...
void Rings() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      // colormapping
      pixel.red   = show1;
      pixel.green = show2 / 4.0f;
      pixel.blue  = show3 / 4.0f;
...
void Caleido2() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_y    = 0.1;
      animation.scale_z    = 0.1;
      animation.offset_y   = 2.0f * move.linear[0];
      animation.offset_x   = 0;
      animation.offset_z   = 0;
      animation.z          = move.linear[0];
      float show1          = render_value(animation);
...
void Caleido3() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_z    = 0.1;
      animation.offset_y   = 2.0f * move.linear[0];
      animation.offset_x   = 2.0f * move.linear[1];
      animation.offset_z   = 0;
      animation.z          = move.linear[0];
      float show1          = render_value(animation);
...
void Lava1() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_x    = 0.15;// + (move.directional[0] + 2)/100;
      animation.scale_y    = 0.12;// + (move.directional[1] + 2)/100;
      animation.offset_x   = show1 / 100.0f;
      ...
      animation.scale_x    = 0.15;// + (move.directional[0] + 2)/100;
      animation.scale_y    = 0.12;// + (move.directional[1] + 2)/100;
      animation.offset_x   = show2 / 100.0f;
      animation.offset_y   += show2/100.0f;
      ...
      pixel.green = 0.1f*linear*(show2-show3);
      pixel.blue = 0;
...
void Scaledemo1() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_y   = 0;
      animation.offset_x   = 0;
      animation.offset_z   = 100.0f*move.linear[0];
      animation.z          = 30;
      float show1          = render_value(animation);
...
void Yves() {
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      pixel.red   = show3;
      pixel.green = show3*show4/255.0f;
      pixel.blue  = 0;
...
void Hot_Blob() { // nice one
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_x   = show3/20.0f;
      animation.offset_y   = -move.linear[0]/2.0f + show1/70.0f;
      animation.low_limit  = 0;
      float show2          = render_value(animation);

      animation.offset_x   = show3/20.0f;
      animation.offset_y   = -move.linear[0]/2 + show1/70.0f;
      animation.z          = 100;
      float show4          = render_value(animation);
      ...
      pixel.green   = linear * radial* 0.3f* (show2-show4);
      pixel.blue = 0;
...
void Slow_Fade() { // nice one
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_y   = -50.0f * move.linear[0];
      animation.offset_x   = 0;
      animation.offset_z   = 0;
      ...
      animation.dist       = animation.dist  * 1.1f;
      animation.angle      += move.noise_angle[0] / 10.0f;
      float show2          = render_value(animation);

      animation.dist       = animation.dist  * 1.1f;
      animation.angle      += move.noise_angle[1] / 10.0f;
      ...
      pixel.green  = radial * (show1 - show2) / 6.0f;
      pixel.blue   = radial * (show1 - show3) / 5.0f;
...
void RGB_Blobs3() { // nice one
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      pixel.blue   = radial * (show3+show2)*0.5f * x/15.0f;
...
void Big_Caleido() { // nice one
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_x    = 0.05;
      animation.scale_y    = 0.05;
      animation.offset_z   = 50.0f * move.linear[1];
      animation.offset_x   = 50.0f * move.noise_angle[1];
      animation.offset_y   = 50.0f * move.noise_angle[2];
      float show2          = render_value(animation);
      ...
      animation.offset_z   = 10.0f * move.linear[2];
      animation.offset_x   = 10.0f * move.noise_angle[2];
      animation.offset_y   = 10.0f * move.noise_angle[3];
      float show3          = render_value(animation);
      ...
      animation.scale_x    = 0.10;
      animation.scale_y    = 0.10;
      animation.offset_z   = 10.0f * move.linear[3];
      animation.offset_x   = 10.0f * move.noise_angle[3];
      animation.offset_y   = 10.0f * move.noise_angle[4];
      float show4          = render_value(animation);
      ...
      animation.scale_x    = 0.15;
      animation.scale_y    = 0.15;
      animation.offset_z   = 10.0f * move.linear[4];
      animation.offset_x   = 10.0f * move.noise_angle[4];
      animation.offset_y   = 10.0f * move.noise_angle[5];
...
void SM1() { // nice one
  ...
  for (int x = 0; x < num_x/2; x++) {
    for (int y = 0; y < num_y/2; y++) {
      ...
      animation.scale_x    = 0.1;
      animation.scale_y    = 0.1;
      animation.offset_z   = 50.0f * move.linear[0];
      animation.offset_x   = 150.0f * move.directional[0];
      ...
      animation.scale_y    = 0.15;
      animation.offset_z   = 50.0f * move.linear[1];
      animation.offset_x   = 150.0f * move.directional[1];
      ...
      animation.scale_x    = 0.1;
      animation.scale_y    = 0.1;
      animation.offset_z   = 50.0f * move.linear[2];
      animation.offset_x   = 150.0f * move.directional[2];
      ...
      animation.scale_x    = 0.15;
      animation.scale_y    = 0.15;
      animation.offset_z   = 50.0f * move.linear[3];
      animation.offset_x   = 150.0f * move.directional[3];
      ...
      animation.scale_x    = 0.2;
      animation.scale_y    = 0.2;
      animation.offset_z   = 50.0f * move.linear[4];
      animation.offset_x   = 150.0f * move.directional[4];
      animation.offset_y   = 150.0f * move.directional[5];
      float show5          = render_value(animation);
...
void SM2() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = 5.0f * move.linear[0];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
      float show1          = render_value(animation);
      ...
      animation.scale_x    = 0.07;
      animation.scale_y    = 0.07;
      animation.offset_z   = 5.0f * move.linear[1];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
      float show2          = render_value(animation);
      ...
      animation.scale_x    = 0.05;
      animation.scale_y    = 0.05;
      animation.offset_z   = 5.0f * move.linear[2];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
      float show3          = render_value(animation);
...
void SM3() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = 0;
      animation.offset_x   = 0;
      animation.offset_y   = -20.0f * move.linear[0];;
      animation.low_limit  = -1;
      animation.high_limit = 1;
      ...
      animation.scale_y    = 0.09;
      animation.offset_z   = 0;
      animation.offset_x   = 500.0f+show1/20.0f;
      animation.offset_y   = -4.0f * move.linear[0] + show2/20.0f;
      animation.low_limit  = 0;
      animation.high_limit = 1;
      ...
      animation.scale_y    = 0.09;
      animation.offset_z   = 0;
      animation.offset_x   = 500.0f+show1/18.0f;
      animation.offset_y   = -4.0f * move.linear[0] + show2/18.0f;
      animation.low_limit  = 0;
      animation.high_limit = 1;
      ...
      animation.scale_y    = 0.09;
      animation.offset_z   = 0;
      animation.offset_x   = 500.0f+show1/19.0f;
      animation.offset_y   = -4.0f * move.linear[0] + show2/19.0f;
...
void SM4() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = 0;
      animation.offset_x   = 0;
      animation.offset_y   = -40.0f * move.linear[0];
      animation.low_limit  = 0;
      animation.high_limit = 1;
      show2          = render_value(animation);
...
void SM5() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_x    = 0.09;
      animation.scale_y    = 0.09;
      animation.offset_z   = 5.0f * move.linear[3];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
      float show4          = render_value(animation);
      ...
      animation.scale_x    = 0.07;
      animation.scale_y    = 0.07;
      animation.offset_z   = 5.0f * move.linear[4];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
      float show5          = render_value(animation);
      ...
      animation.scale_x    = 0.05;
      animation.scale_y    = 0.05;
      animation.offset_z   = 5.0f * move.linear[5];
      animation.offset_x   = 0;
      animation.offset_y   = 0;
...
void SM9() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_x    = 0.09;
      animation.scale_y    = 0.09;
      animation.offset_y   = -30.0f * move.linear[0];
      animation.offset_z   = 0;
      animation.offset_x   = 0;
      animation.low_limit  = -1;
      show1                = render_value(animation);
      ...
      animation.scale_x    = 0.09;
      animation.scale_y    = 0.09;
      animation.offset_y   = -30.0f * move.linear[1];
      animation.offset_z   = 0;
      animation.offset_x   = 0;
      animation.low_limit  = -1;
      show2                = render_value(animation);
      ...
      animation.scale_x    = 0.09;
      animation.scale_y    = 0.09;
      animation.offset_y   = -10.0f * move.linear[0];
      animation.offset_z   = 0;
      animation.offset_x   = 0;
      animation.low_limit  = 0;
      show3                = render_value(animation);
...
void SM10() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      pixel.red    = (show5+show6)/2.0f;
      pixel.green  = (show5-50.0f)+(show6/16.0f);
      pixel.blue   = 0;//show6;
...
void Complex_Kaleido() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_y    = 0.07;
      animation.offset_z   = 0;
      animation.offset_x   = -30.0f * move.linear[0];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show1                = render_value(animation);
      ...
      animation.scale_y    = 0.07;
      animation.offset_z   = 0;
      animation.offset_x   = -30.0f * move.linear[1];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show2                = render_value(animation);
      ...
      animation.scale_y    = 0.05;
      animation.offset_z   = 0;
      animation.offset_x   = -40.0f * move.linear[2];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show3                = render_value(animation);
      ...
      animation.scale_y    = 0.09;
      animation.offset_z   = 0;
      animation.offset_x   = -35.0f * move.linear[3];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show4                = render_value(animation);
...
void Complex_Kaleido_3() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = -12.0f * move.linear[1];;
      ...
      animation.offset_z   = -12.0f * move.linear[3];
      animation.offset_x   = -40.0f * move.linear[3];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show3                = render_value(animation);
      ...
      //pixel.red    = radial*(show1+show2);
      pixel.green  = 0.2f*show8;//(radial*(show1))*0.3f;
      pixel.blue   = show5 * radial;
      pixel.red    = (show1 + show2) - show7/2.0f;
...
void Complex_Kaleido_4() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = -10.0f * move.linear[0];
      ...
      animation.offset_z   = -10.0f * move.linear[1];
      animation.offset_x   = 0;//-30 * move.linear[0];
      animation.offset_y   = 0;
      animation.low_limit  = 0;
      show2                = render_value(animation);
      ...
      animation.scale_y    = 0.2f * size ;
      animation.offset_z   = 0;//-12 * move.linear[3];
      animation.offset_y   = +7.0f * move.linear[3] +  move.noise_angle[3];
      animation.offset_x   = 0;
      animation.low_limit  = 0;
      show3                = render_value(animation);
      ...
      */
      ...
      pixel.blue   = (64.0f-show5-show3) * radial;
      pixel.green  = 0.5f*(show6);
      //pixel.blue   = show5 * radial;
...
void Complex_Kaleido_6() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = -10.0f * move.linear[0];
      animation.offset_y   = 10.0f * move.noise_angle[0];
      animation.offset_x   = 10.0f * move.noise_angle[4];
      animation.low_limit  = 0;
      show1                = render_value(animation);
      ...
      animation.scale_y    = 0.06 ;
      animation.offset_z   = -10.0f * move.linear[1];
      animation.offset_y   = 10.0f * move.noise_angle[1];
      animation.offset_x   = 10.0f * move.noise_angle[3];
      animation.low_limit  = 0;
      show2                = render_value(animation);
...
void Water() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_y    = 0.1 ;
      animation.offset_z   = -10 ;
      animation.offset_y   = 20.0f * move.linear[1];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show3                = render_value(animation);
      ...
      animation.scale_y    = 0.1 ;
      animation.offset_z   = -10 ;
      animation.offset_y   = 20.0f * move.linear[2];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show4                = render_value(animation);
      ...
      pixel.blue    = (0.7f*show2+0.6f*show3+0.5f*show4);
      pixel.red     = pixel.blue-40.0f;
      pixel.green   = 0;
...
void Parametric_Water() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_y    = 0.1 ;
      animation.offset_z   = -10 ;
      animation.offset_y   = 20.0f * move.linear[3];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show5                = render_value(animation);
      ...
      pixel.red     = pixel.blue-40.0f;
      pixel.green   = 0;
      pixel.blue    = (0.3f*show6+0.7f*show7)*radial;
...
void Module_Experiment2() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      pixel.red    = show1;
      pixel.green  = show1 - 80.0f;
      pixel.blue   = show1 - 150.0f;
...
void Zoom2() { // nice one
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_y   = -10.0f*move.linear[0];
      animation.offset_x   = 0;
      animation.offset_z   = 0.1f*move.linear[0];
...
void Module_Experiment4() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.offset_z   = 0.1f*move.linear[2] ;
      animation.offset_y   = -20.0f * move.linear[2];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show1                = render_value(animation);
      ...
      animation.scale_x    = 0.004f * s;
      animation.scale_y    = 0.003f * s;
      animation.offset_z   = 0.1f*move.linear[3] ;
      animation.offset_y   = -20.0f * move.linear[3];
      ...
      animation.scale_x    = 0.004f * s;
      animation.scale_y    = 0.003f * s;
      animation.offset_z   = 0.1f*move.linear[4] ;
      animation.offset_y   = -20.0f * move.linear[4];
...
void Module_Experiment6() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_x    = 0.1f * s;
      animation.scale_y    = 0.1f * s;
      animation.offset_z   = 0.1f*move.linear[1] ;
      animation.offset_y   = -20.0f * move.linear[1];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show2                = render_value(animation);
      ...
      */
      ...
      pixel.red    = (show1 + show2);
      pixel.green  = ((show1 + show2)*0.6f)-30.0f;
      pixel.blue   = 0;
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      */
      ...
      pixel.red    = (show1 + show2);
      pixel.green  = ((show1 + show2)*0.6f)-50.0f;
      pixel.blue   = 0;
...
void Module_Experiment8() { 
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      animation.scale_y    = 0.1f * s;
      animation.offset_z   = 10.0f*move.linear[0] ;
      animation.offset_y   = -5.0f * r * move.linear[0];
      animation.offset_x   = 10;
      animation.low_limit  = 0;
      show1                = render_value(animation);
      ...
      animation.scale_y    = 0.1f * s;
      animation.offset_z   = 0.1f*move.linear[1] ;
      animation.offset_y   = -5.0f * r * move.linear[1];
      animation.offset_x   = 100;
      animation.low_limit  = 0;
      show2                = render_value(animation);
      ...
      animation.scale_y    = 0.1f * s;
      animation.offset_z   = 0.1f*move.linear[2] ;
      animation.offset_y   = -5.0f * r * move.linear[2];
      animation.offset_x   = 1000;
      animation.low_limit  = 0;
      show3                = render_value(animation);
      ...
      */
      ...
      pixel.green  = (((show2 + show3)*0.8f)-90.0f)*rad;
  ...
  for (int x = 0; x < num_x; x++) {
    for (int y = 0; y < num_y; y++) {
      ...
      */
      ...
      // CHSV(rad * ((show1 + show2) + show3), 255, 255); // BUG this does nothing !!!
      //CRGB fpix= CHSV(rad * ((show1 + show2) + show3), 255, 255);
      //pixel.red   = fpix.red;
      //pixel.green = fpix.green;
      //pixel.blue = fpix.blue;      
